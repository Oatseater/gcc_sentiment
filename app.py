...
# Clean, opinionated business tool — Inter + JetBrains Mono. No serif. No poetry.
st.markdown("""
<link href="https://fonts.googleapis.com/css2?family=Inter:wght@300;400;500;600&family=JetBrains+Mono:wght@400;500&family=Noto+Naskh+Arabic:wght@400;500&display=swap" rel="stylesheet">
<style>
  html, body, [class*="css"] { font-family: 'Inter', system-ui, sans-serif; background: #0f0f0f; color: #e2e2e2; }
  ...
  .app-name {
    font-size: 0.85rem; font-weight: 600; color: #e2e2e2;
    letter-spacing: 0.01em;
  }
  .app-tag {
    font-family: 'JetBrains Mono', monospace;
    font-size: 0.65rem; color: #555; background: #1a1a1a;
    border: 1px solid #2a2a2a; padding: 0.15rem 0.5rem;
  ...
  .stTabs [data-baseweb="tab-list"] {
    background: transparent !important; border-bottom: 1px solid #232323 !important; gap: 0;
  }
  .stTabs [data-baseweb="tab"] {
    ...
    color: #555 !important; padding: 0.55rem 1.2rem !important;
  ...
  /* Section header */
  .sec { font-family: 'JetBrains Mono', monospace; font-size: 0.6rem; color: #444;
  ...
  .stat-cell {
    flex: 1; background: #0f0f0f; padding: 0.9rem 1rem;
  ...
  .dt th {
    font-family: 'JetBrains Mono', monospace; font-size: 0.58rem; color: #444;
    text-transform: uppercase; letter-spacing: 0.1em;
    padding: 0.4rem 0.6rem 0.5rem; border-bottom: 1px solid #232323; text-align: left;
  }
  ...
  .asp-row {
    ...
    padding: 0.4rem 0; border-bottom: 1px solid #1a1a1a; font-size: 0.78rem;
  }
  ...
  /* Review cards */
  .rev-card {
    border-left: 2px solid #232323; padding: 0.6rem 0 0.6rem 0.9rem;
    margin-bottom: 0.6rem; font-size: 0.8rem; color: #999; line-height: 1.55;
  }
  ...
  .ip-high { color: #f87171; } .ip-med { color: #fbbf24; } .ip-low { color: #4ade80; }
  ...
  /* Inputs */
  .stTextArea textarea {
    background: #141414 !important; border: 1px solid #2a2a2a !important; border-radius: 3px !important;
    color: #e2e2e2 !important; font-family: 'Inter', sans-serif !important; font-size: 0.85rem !important;
  }
  ...
  .stSelectbox > div > div {
    ...
    border-radius: 3px !important; color: #e2e2e2 !important;
  }
  .stButton > button {
    ...
    font-size: 0.65rem !important; letter-spacing: 0.1em !important; text-transform: uppercase !important;
  ...
  ::-webkit-scrollbar { width: 3px; } ::-webkit-scrollbar-track { background: #0f0f0f; } ::-webkit-scrollbar-thumb { background: #2a2a2a; }
</style>
...
def detect_lang(text: str) -> str:
    ...
    return 'ar' if ar / total > 0.7 else 'en'

def is_arabic(text: str) -> bool:
    return detect_lang(str(text)) == 'ar'

def heuristic_score(text: str):
    neg_kw = ['متأخر','بارد','مخيب','سيء','فظيع','terrible','awful','crashed','bad','worst','poor','cold','late','wrong','broken','never','avoid','rude','slow','disgusting','horrible']
    pos_kw = ['ممتاز','رائع','excellent','amazing','love','great','best','fast','perfect','phenomenal','good','clean','helpful','recommend','happy','smooth','impressed']
    t = text.lower()
...
def batch_heuristic(df: pd.DataFrame, text_col: str) -> pd.DataFrame:
    rows = []
    for _, row in df.iterrows():
        text = str(row.get(text_col, ''))
        sent, conf = heuristic_score(text)
        lang = 'arabic' if is_arabic(text) else 'english'
        nr = row.to_dict()
        nr.update({'sentiment': sent, 'confidence': round(conf, 3), 'language': lang, 'model': 'heuristic'})
        rows.append(nr)
    return pd.DataFrame(rows)

def asp_html(aspects: dict) -> str:
    cls = {'positive':'ab-pos','negative':'ab-neg','neutral':'ab-neu'}
    out = ''
    for asp, sent in sorted(aspects.items(), key=lambda x: {'negative':0,'neutral':1,'positive':2}[x[1]]):
        out += f'<div class="asp-row"><span class="asp-name">{asp}</span><span class="asp-badge {cls.get(sent,"ab-neu")}">{sent}</span></div>'
    return out

def ins_html(insights: list) -> str:
...
<div class="ins-p {pc.get(p,'ip-low')}">{p}</div>
<div class="ins-f">{ins.get('finding','')}</div>
<div class="ins-r">{ins.get('recommendation','')}</div>
<div class="ins-m">{ins.get('metric','')}</div>
</div>"""
    return out

def freq_html(en_freq, ar_freq):
    rows = ''
    for i in range(min(max(len(en_freq), len(ar_freq)), 10)):
        n = f'{i+1:02d}'
        ew = en_freq[i][0] if i < len(en_freq) else ''
        ec = en_freq[i][1] if i < len(en_freq) else ''
        aw = ar_freq[i][0] if i < len(ar_freq) else ''
        ac = ar_freq[i][1] if i < len(ar_freq) else ''
        rows += (f'<tr><td class="mono" style="color:#444">{n}</td>'
                 f'<td>{ew}</td><td class="mono r" style="color:#444">{ec}</td>'
                 f'<td style="width:1.5rem"></td>'
                 f'<td class="mono" style="color:#444">{n}</td>'
                 f'<td style="font-family:\'Noto Naskh Arabic\',serif;direction:rtl;text-align:right">{aw}</td>'
                 f'<td class="mono r" style="color:#444">{ac}</td></tr>')
    return f'<table class="dt"><thead><tr><th colspan="3">English</th><th></th><th colspan="3" style="text-align:right">Arabic</th></tr></thead><tbody>{rows}</tbody></table>'

def scorecard_html(brand_scores, comp_df):
    ...
    for _, row in brand_scores.iterrows():
        ...
        rows += (f'<tr><td class="brand">{row["brand"]}</td>'
                 ...
                 f'<td class="mono r {vel_cls}">{vel}</td></tr>')
    return f'''<table class="dt">
<thead><tr>
<th>Brand</th><th class="r">Score</th><th class="r">Pos%</th>
<th class="r">Neg%</th><th class="r">Neu%</th><th class="r">n</th><th class="r">Trend</th>
...
with tab1:
    ...
    with c_in:
        ...
        user_text = st.text_area("Review text", value=default, height=120,
                                  placeholder="Paste any review — Arabic, English, or mixed.", label_visibility="collapsed")
        ...
        if btn and user_text.strip():
            with st.spinner(""):
                try:
                    ...
                    sent, conf = heuristic_score(user_text.strip())
                    from analysis.aspect_extractor import extract_aspects
                    lang = 'arabic' if is_arabic(user_text) else 'english'
                    result = {'sentiment': sent, 'confidence': conf, 'language': lang,
                              'model': 'heuristic', 'emotion': '', 'aspects': extract_aspects(user_text, lang),
                              'label_scores': {}}
                    mode = 'heuristic'
            ...
            if aspects:
                ...
                st.markdown(asp_html(aspects), unsafe_allow_html=True)

        elif btn:
            st.markdown('<div class="conf-line">No input.</div>', unsafe_allow_html=True)
        else:
            st.markdown('''
        ...
        if 'batch_df' in st.session_state:
            ...
            with st.spinner("Scoring…"):
                try:
                    ...
                    scored = batch_heuristic(df_raw, text_col)
            ...
            for _, row in scored.head(8).iterrows():
                ...
                ex = str(row[text_col])[:65] + '…' if len(str(row[text_col])) > 65 else str(row[text_col])
                rows_html += f'<tr><td>{ex}</td><td class="mono r {cls}">{sent}</td><td class="mono r" style="color:#444">{float(row["confidence"]):.3f}</td></tr>'
            st.markdown(f'<table class="dt"><thead><tr><th>Text</th><th class="r">Sentiment</th><th class="r">Conf</th></tr></thead><tbody>{rows_html}</tbody></table>', unsafe_allow_html=True)

            csv_out = scored.to_csv(index=False).encode('utf-8-sig')
            st.download_button("Download scored CSV", data=csv_out,
                               file_name=f"scored_{datetime.now().strftime('%Y%m%d')}.csv", mime='text/csv')
...
with tab3:
    ...
    with c_sel:
        ...
        st.markdown('<div class="conf-line" style="margin-top:0.4rem">Reddit · Google News RSS<br>No API key required</div>', unsafe_allow_html=True)
    ...
    with c_main:
        ...
        if live_btn:
            with st.spinner(f"Fetching {brand_sel}…"):
                ...
                if live_items:
                    ...
                    ldf = batch_heuristic(ldf, 'text')
                    ldf['brand'] = brand_sel
                    ldf['timestamp'] = datetime.now().isoformat()
                    brand_df = pd.concat([base_df[base_df['brand']==brand_sel], ldf], ignore_index=True)
        ...
        brand_df = st.session_state.get(f'live_{brand_sel}', base_df[base_df['brand']==brand_sel].copy())

        if brand_df.empty:
            st.markdown('<div class="conf-line">No data.</div>', unsafe_allow_html=True)
        else:
            ...
            conf  = brand_df['confidence'].mean() if 'confidence' in brand_df.columns else 0.0
            ...
            if not timeline.empty:
                ...
                st.markdown(f'<div class="sec" style="margin-top:1rem">Sentiment trend &nbsp;<span class="{vel_cls}" style="font-family:JetBrains Mono,monospace;font-size:0.6rem">{vel_label} Δ{vel_val:+.3f}</span></div>', unsafe_allow_html=True)
            ...
            with ca:
                ...
                for _, row in brand_df.head(60).iterrows():
                    for asp, sent in extract_aspects(str(row['text']), row.get('language','english')).items():
                ...
                st.markdown(asp_html(agg), unsafe_allow_html=True)
            ...
            with cw:
                ...
                st.markdown(freq_html(en_freq, ar_freq), unsafe_allow_html=True)
            ...
            with cp:
                ...
                for q in get_top_reviews(brand_df, brand_sel, 'positive', 3):
                    cls = 'rev-ar' if is_arabic(q) else ''
                    st.markdown(f'<div class="rev-card pos {cls}">{q[:200]}</div>', unsafe_allow_html=True)
            with cn:
                st.markdown('<div class="sec">Top negative</div>', unsafe_allow_html=True)
                for q in get_top_reviews(brand_df, brand_sel, 'negative', 3):
                    cls = 'rev-ar' if is_arabic(q) else ''
                    st.markdown(f'<div class="rev-card neg {cls}">{q[:200]}</div>', unsafe_allow_html=True)
            ...
            st.markdown(ins_html(insights) if insights else '<div class="conf-line">Insufficient data.</div>', unsafe_allow_html=True)
...
with tab4:
    ...
    if brand_a == brand_b:
        ...
        st.markdown(scorecard_html(brand_scores, comp_df), unsafe_allow_html=True)
        ...
        for col_c, bn in [(tc1, brand_a), (tc2, brand_b)]:
            with col_c:
                ...
                if not tl.empty:
                    st.plotly_chart(sentiment_trend_line(tl, bn), use_container_width=True, config={'displayModeBar': False})
        ...
        for col_w, bn in [(wc1, brand_a), (wc2, brand_b)]:
            with col_w:
                ...
                st.markdown(freq_html(get_word_frequency(comp_df, bn, 'english', 8),
                                      get_word_frequency(comp_df, bn, 'arabic', 8)), unsafe_allow_html=True)
        ...
        if not all_scores.empty:
            st.plotly_chart(brand_comparison_chart(all_scores), use_container_width=True, config={'displayModeBar': False})

# Footer
st.markdown("""
<hr class="div" style="margin-top:2rem">
<div style="display:flex;justify-content:space-between;padding:0.4rem 0">
  <span class="conf-line">GCC Sentiment Intelligence · UAE Market · AR+EN</span>
